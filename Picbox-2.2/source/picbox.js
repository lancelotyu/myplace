	...
	*/
    ...
    $(document).ready(function () {
        $(parent.document.body).append(
			$([
				...
				bottom = $('<div id="pbBottom" />').append([
					...
					$('<div id="pbNav" />').append([
						prevBtn = $('<a id="pbPrevBtn" title ="前一张图片" href="#" />').click(previous)[0],
                        leftBtn = $('<a id="pbLeftBtn" title ="向左旋转图片" href="#" />').click(rotateleft)[0],
						zoomBtn = $('<a id="pbZoomBtn" title ="缩放图片" href="#" />').click(doubleClick)[0],
                        rightBtn = $('<a id="pbRightBtn" title ="向右旋转图片" href="#" />').click(rotateright)[0],
						nextBtn = $('<a id="pbNextBtn" title ="后一张图片" href="#" />').click(next)[0]
					])[0],
        ...
        $(image).tinyDrag(function () {
            ...
            $(zoomBtn).attr("title", "缩放到合适大小");
        });
    });

    $.picbox = function (_images, startImage, _options) {
        options = $.extend({
            loop: false,					// 允许浏览第一个和最后一个图片之间的。Allows to navigate between first and last images
            overlayOpacity: 0.8,			// 1是不透明的，0是完全透明的(可以在css文件中修改颜色)。1 is opaque, 0 is completely transparent (change the color in the CSS file)
            overlayFadeDuration: 500,		// 淡入和淡出的动画叠加的持续时间(以毫秒为单位)。Duration of the overlay fade-in and fade-out animations (in milliseconds)
            resizeDuration: 500,			// 每一个的图像大小调整动画的持续的时间(以毫秒为单位)。Duration of each of the image resize animations (in milliseconds)
            resizeEasing: "swing",			// swing 用Jquery默认的设置。swing uses the jQuery default easing
            controlsFadeDelay: 3000,		// 延时控制不移动时鼠标之前的按钮亮暗（以毫秒为单位）.Time delay before controls fade when not moving the mouse (in milliseconds)
            counterText: false,				// 计数文本，用{x}记录当前图像，{y}记录总数，例如 image{x}of{y}。Counter text. Use {x} for current image and {y} for total e.g. Image {x} of {y}
            hideFlash: true,				// 隐藏flash上的元素被激活picbox时。注：flash元素必须有wmode参数设置为“不透明”或“透明”的，如果设置为false。Hides flash elements on the page when picbox is activated. NOTE: flash elements must have wmode parameter set to "opaque" or "transparent" if this is set to false
            closeKeys: [27, 88, 67],		// 使用键盘关闭的键的数组。默认是ESC。Array of keycodes to close Picbox, default: Esc (27), 'x' (88), 'c' (67)
            previousKeys: [37, 80],			// 使用键盘上一页的键的数组。Array of keycodes to navigate to the previous image, default: Left arrow (37), 'p' (80)
            nextKeys: [39, 78],				// 使用键盘下一页的键的数组。Array of keycodes to navigate to the next image, default: Right arrow (39), 'n' (78)
            leftKeys: [38],
            rightKeys: [40],
            margins: 0						// 图像和窗口间的边距(像素为单位)。Margin between the image and the sides of the window (in pixels)
        }, _options || {});
        ...
        $(bottom).css("display", "");
        mouseMove(); // 不动的时候鼠标自动隐藏。So controls dissapear if even if mouse is never moved
        position();
        setup(1);
        //add by lancelot 清空当前图像的ratate
        $(image).css("rotate", "0deg");
        _rotate = 0;
        //
        images = _images;
    ...
    $.fn.picbox = function (_options, linkMapper, linksFilter) {
        linkMapper = linkMapper || function (el) {
            ...
            //return [el.href, el.title]; //于嘉涛修改，由于img的herf是src
        };
	...
	*/
    ...
    function keyDown(event) {
        ...
        return $.inArray(code, options.closeKeys) >= 0 ? close()
            ...
            : $.inArray(code, options.leftKeys) >= 0 ? rotateleft()
            : $.inArray(code, options.rightKeys) >= 0 ? rotateright()
			: false;
    }

    function mouseMove() {
        flashFade([bottom, prevBtn, zoomBtn, nextBtn, rightBtn, leftBtn]);
    }
    ...
    //add by lancelot_yu @ 2013.05.17 NOTE: ADD ROTATE IMAGE FUCTION
    function rotateleft() {
        return rotateImage('left');
    }

    function rotateright() {
        return rotateImage('right');
    }
    ...
    function rotateImage(to) {
        //alert(to);
        if (to == "right") {
            _rotate += 90;
			alert(ObjectToString(image));
            $(image).animate({ rotate: _rotate + 'deg' }, 500);
        }
        //alert(to);
        if (to == "left") {
            _rotate -= 90;
    ...
    function stop() {
        ...
        $(image).stop();
        $([prevBtn, nextBtn, leftBtn, rightBtn]).addClass(greyed);
        $(zoomBtn).removeClass(zoomed);
        $(zoomBtn).attr("title", "缩放图片");
    }
